...
public class user extends JFrame {
	...
	public user( String name) {
		...
		JLabel lblNewLabel_6 = new JLabel("Books highlighted in yellow are borrowed by you ");
		...
		JLabel lblNewLabel_1 = new JLabel("Book Available");
...
//
		ImageIcon imageIcon = new ImageIcon(new ImageIcon("./res/background.jpg").getImage().getScaledInstance(1000, 1000, Image.SCALE_SMOOTH));
		//
		...
		ImageIcon imagebtn1 = new ImageIcon(new ImageIcon("./res/button1.png").getImage().getScaledInstance(200, 100, Image.SCALE_SMOOTH));
		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			{
				dispose();
				new change_pass(name).setVisible(true);
			}
		});
		...
		try {
            ...
            String q="select st_name,st_pass from studenttable where st_name='"+name+"'";
...
//            textFieldid.setText(st_id);
		...
		String columns[]= {"Book_id","Book_Name","Author","Published Date","Price"};
		
		int rowsize=0;
		try {
			conn c=conn.getInstance();
			String q1="select COUNT(*) from booktable";
		...
		try {
			conn c1=conn.getInstance();
			//String q2="select book_id,book_name,author from booktable where stid="+stid;
			String q2="select book_id,book_name,author,duedate,fine,request from booktable";
...
//		contentPane.add(lblNewLabel_2);
		...
		JButton btn_return = new JButton("Request BOOK");
		...
		ImageIcon imagebtn = new ImageIcon(new ImageIcon("./res/button.png").getImage().getScaledInstance(200, 100, Image.SCALE_SMOOTH));
